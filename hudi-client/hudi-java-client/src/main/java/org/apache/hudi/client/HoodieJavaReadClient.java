 ...
 * consumed, so only one file group's merge buffer is open at a time. File-slice planning still uses
 * memory proportional to the number of selected file slices.
 *
 ...
 *   <li>LSM tree storage requires a different file-group reader and is rejected at the read entry points.</li>
 *   <li>This API returns merged records. CDC-enabled writes can be consumed through the shared CDC
 *   extractor; this client does not expose a CDC read method.</li>
 * </ul>
 */
public class HoodieJavaReadClient implements AutoCloseable {
  ...
  public HoodieJavaReadClient(HoodieJavaEngineContext context, HoodieTableMetaClient metaClient, TypedProperties props) {
    ...
    this.metadataConfig = HoodieMetadataConfig.newBuilder().fromProperties(this.props).build();
  }
  ...
  public ClosableIterator<HoodieRecord<IndexedRecord>> readSnapshot(String asOfInstant) {
    ...
    if (freshMetaClient.getTableConfig().isLSMTreeStorageLayout()) {
      throw new UnsupportedOperationException("HoodieJavaReadClient does not yet support LSM tree storage layout");
    }
    ...
    if (timeline.empty() && asOfInstant == null) {
      return new EmptyIterator<>();
    }
    String maxInstantTime;
    if (asOfInstant != null) {
      ValidationUtils.checkArgument(timeline.filterCompletedInstants().getInstantsAsStream().anyMatch(i -> i.requestedTime().equals(asOfInstant)),
  ...
  public ClosableIterator<HoodieRecord<IndexedRecord>> readIncremental(String startCompletionTime, String endCompletionTime,
    ...
    if (!freshMetaClient.getTableConfig().populateMetaFields()) {
      throw new UnsupportedOperationException("Incremental reads require hoodie.populate.meta.fields=true");
    }
    ...
    if (queryContext.isConsumingFromEarliest() && queryContext.isConsumingToLatest()) {
      // Compaction can be newer than the last selected delta commit, including after rollback.
      // The unbounded earliest query promises the current snapshot, so use all current file slices.
      HoodieTimeline snapshotTimeline = freshMetaClient.getCommitsAndCompactionTimeline().filterCompletedAndCompactionInstants();
      String latestInstant = snapshotTimeline.filterCompletedInstants().lastInstant().get().requestedTime();
      return toRecordIterator(freshMetaClient,
          collectMergedFileSlices(freshMetaClient, snapshotTimeline, listPartitions(freshMetaClient), latestInstant),
          latestInstant, Option.empty());
    }
  ...
  private ClosableIterator<HoodieRecord<IndexedRecord>> toRecordIterator(HoodieTableMetaClient freshMetaClient, List<FileSlice> fileSlices,
    ...
    Option<InternalSchema> internalSchema = new TableSchemaResolver(freshMetaClient).getTableInternalSchemaFromCommitMetadata();
    List<Supplier<ClosableIterator<HoodieRecord<IndexedRecord>>>> suppliers = new ArrayList<>(fileSlices.size());
    for (FileSlice fileSlice : fileSlices) {
      suppliers.add(() -> openFileSlice(freshMetaClient, fileSlice, schema, internalSchema, latestCommitTime, instantRangeOpt));
    }
  ...
  private ClosableIterator<HoodieRecord<IndexedRecord>> openFileSlice(HoodieTableMetaClient freshMetaClient, FileSlice fileSlice,
                                                                       HoodieSchema schema, Option<InternalSchema> internalSchema,
                                                                       String latestCommitTime, Option<InstantRange> instantRangeOpt) {
